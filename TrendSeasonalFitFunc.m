function rec_cg = TrendSeasonalFitFunc(yeardoy, line_t, ncols,nrows,T_cg,Tmax_cg,conse,num_c,nbands,B_detect)
...
%
%% Inputs:
% yeardoy = ['2019032'; '2019038'; '2019045']; % YearDOY string per row.
% 
% line_t is a array of size (num_t, nbands*ncols), each row is data from one
% day, in the order of nbands for column 1, then nbands for column 2 and so on. 
% 
...
sdate = datenum(str2num(yeardoy(:, 1:4)), 1, 0) + str2num(yeardoy(:, 5:7));
% sort according to datenum
[~, sort_order] = sort(sdate);
yeardoy = yeardoy(sort_order, :);
sdate = sdate(sort_order);
line_t = line_t(sort_order, :);
% number of dates 
num_t = size(line_t,1);
% consecutive number
def_conse = conse;
